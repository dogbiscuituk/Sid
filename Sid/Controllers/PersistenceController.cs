    ...
    public class PersistenceController : SdiController
	{
        ...
        protected override bool LoadFromStream(Stream stream, string format)
		{
			...
			*/
			var result =
				IsXml(format)
					? UseStream(() => Model.Graph = (Graph)GetXmlSerializer().Deserialize(new XmlTextReader(stream)))
					: UseStream(() => Model.Graph = (Graph)GetBinaryFormatter().Deserialize(stream));
		...
		protected override bool SaveToStream(Stream stream, string format)
		{
			return
				IsXml(format)
					? UseStream(() => GetXmlSerializer().Serialize(stream, Model.Graph))
					: UseStream(() => GetBinaryFormatter().Serialize(stream, Model.Graph));
		}

        private static BinaryFormatter GetBinaryFormatter() => new BinaryFormatter();

        private static XmlSerializer GetXmlSerializer() => new XmlSerializer(typeof(Graph));
